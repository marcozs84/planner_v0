 ...
 */
	...
	if ($res) {
		...
		print "{\"result\":\"FALSE\",\"message\":\"Failed to get Timelines. Error: " . $mysqli->error . "\",\"package\":\"null\"}";
		die();
	}
	...
	if ($resTask) {
		if ($resTask->num_rows > 0) {
			...
			while ( $row = $resTask->fetch_assoc () ) {
				...
				if($resSplit){
					...
					$tasks = json_encode($addTasks);

				}else{
					print "{\"result\":\"FALSE\",\"message\":\"Failed to get Splits of tasks. Error: " . $mysqli->error . "\",\"package\":\"null\"}";
					die();
				}
			}
		}
		...
		print "{\"result\":\"FALSE\",\"message\":\"Failed to get Tasks. Error: " . $mysqli->error . "\",\"package\":\"null\"}";
		die();
	}
